...
subtitle: "Opponent and Coin Toss Play Key Role in Deciding Match Outcome"
author: 
...
We employ a Bayesian Logistic Regression model to analyze match outcomes,
...
specification and justification for a Bayesian Logistic Regression model.
...
`modelsummary` [@modelsummary], `knitr` [@knitr], `kableExtra` [@kableExtra] and `ggplot2` [@ggplot2].
...
@fig-match-stats show key match statistics for Pakistan. The data shows that Pakistan frequently faces England, Sri Lanka, and Australia, with a particularly poor record against Australia, including several draws.
...
presenting the predicted values of the coefficients. The figure also
